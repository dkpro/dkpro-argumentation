...
/**
 * A {@link Map} which decorates another {@link Map}, which has {@link IntCollection collections} of
 * values for each key.
 *
 * @param <K>
 *            The key type.
    ...
    implements Map<K, C>, Serializable
{
            ...
            final int toValue)
    {
        ...
        for (final K keyToIncrement : keysToIncrement) {
            incrementValues(multimap, keyToIncrement, increment, fromValue, toValue,
                    incrementedValues);
        }
    }

    /**
     * Adds one value for each given key, incrementing the value added for the next key by one.
     *
     * @param multimap
     *            The {@link MultiValueObject2IntMap} to add to.
     * @param keysToAdd
    ...
    public static final <K, C extends IntCollection> boolean putIncrementingValues(
            final MultiValueObject2IntMap<K, C> multimap, final Iterable<? extends K> keysToAdd,
    ...
    private static final <K, C extends IntSortedSet> void incrementValues(
            ...
            final int increment, final int fromValue, final int toValue,
            final IntCollection incrementedValues)
    {
        // Filter out the values outside the specified range of values to update
        final IntIterable valuesToIncrement = multimap.get(keyToIncrement).subSet(fromValue,
                toValue);
        for (final int valueToIncrement : valuesToIncrement) {
            if (!incrementedValues.contains(valueToIncrement)) {
                ...
                final boolean wasOldValueRemoved = multimap.removeValue(keyToIncrement,
                        valueToIncrement);
    ...
    /**
     * A {@link IntCollection} of all elements for all keys in the {@link #getDecorated() decorated
     * map}.
     */
    ...
    /**
     * The {@link Factory} used for creating new value collections for the map keys.
     */
     ...
     *            The {@link Factory} to use for creating new value collections for the map keys.
     */
    public MultiValueObject2IntMap(final Map<K, C> decorated,
            final Supplier<? extends C> valueCollectionFactory)
    {
    ...
    /**
     * @return An unmodifiable view of a {@link IntCollection} of all elements for all keys in the
     ...
     * @return A {@link IntCollection} of elements; if there is no mapping for the given key, then
     *         an empty {@code IntCollection} is returned.
     */
     ...
     * @param value
     *            The value to remove from the {@link IntCollection} of values mapped to the given
     *            key.
     ...
     *            The key to remove the given mapped value for.
     * @param values
     *            The values to remove from the {@link IntCollection} of values mapped to the given
     *            key.
    ...
    public String toString()
    {
        ...
        final StringBuilder builder = new StringBuilder(
                prefix.length() + decoratedRepr.length() + 1);
