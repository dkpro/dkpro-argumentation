...
/**
 * A class representing a <a href="https://en.wikipedia.org/wiki/Directed_acyclic_graph">directed
 ...
 *
 * @param <T>
 *            The concrete type of {@code SpanTextLabel} used returned by methods in this class.
 *
 */
public final class SpanAnnotationGraph<T extends SpanTextLabel>
{
            ...
            final int size)
    {
        ...
        for (final Int2ObjectMap.Entry<? extends Int2ObjectMap<? extends Map<?, T>>> firstDimEntry : matrixRows) {
            for (final Int2ObjectMap.Entry<? extends Map<?, T>> secondDimEntry : firstDimEntry
                    .getValue().int2ObjectEntrySet()) {
    ...
    @JsonCreator
    public SpanAnnotationGraph(
            @JsonProperty(PROPERTY_SPAN_ANNOTATIONS) final List<T> spanAnnotationVector,
            @JsonProperty(PROPERTY_RELATIONS) final int[] relationTransitionTable)
    {
        this(spanAnnotationVector instanceof ReverseLookupOrderedSet
                ? (ReverseLookupOrderedSet<T>) spanAnnotationVector
                : new ReverseLookupOrderedSet<>(spanAnnotationVector),
                SpanAnnotationMatrices.createMatrix(spanAnnotationVector).getBackingMap(),
            ...
            final int[] relationTransitionTable)
    {
        this(spanAnnotationVector instanceof ReverseLookupOrderedSet
                ? (ReverseLookupOrderedSet<T>) spanAnnotationVector
                : new ReverseLookupOrderedSet<>(spanAnnotationVector), spanAnnotationMatrix,
                relationTransitionTable);
    }
    ...
    public Map<String, T> getLabels(final Span span)
    {
        ...
        if (firstDim == annotMatrix.defaultReturnValue()) {
            throw new NoSuchElementException(
                    String.format("Span begin index %d not found in matrix.", begin));
        }
        final int end = span.getEnd();
        result = firstDim.get(end);
        if (result == firstDim.defaultReturnValue()) {
            throw new NoSuchElementException(
                    String.format("Span end index %d not found in matrix.", end));
        }
        return result;
    ...
    public int hashCode()
    {
        ...
        result = prime * result
                + (spanAnnotationVector == null ? 0 : spanAnnotationVector.hashCode());
        return result;
    }
